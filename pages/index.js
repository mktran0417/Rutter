...
async function fetchConnection(accessToken) {
  const result = await axios.post("/api/rutter-get-connection", {
    accessToken,
  });
  return result;
}
...
export default function Home() {
  ...
  const handler = async (rutter_request) => {
    setDataFetched(null);
    setLoading(true);
    try {
      const request = await rutter_request(accessToken);
      setDataFetched(request);
    } catch (e) {
      setErrorMessage(e);
    } finally {
      setLoading(false);
        ...
        <Table style={{ marginTop: 8 }} striped bordered hover>
          ...
          <tbody>
            <tr>
              ...
              <td>/orders</td>
              <td>
                <Button onClick={() => handler(fetchOrders)}>Send request</Button>
              </td>
            </tr>
            <tr>
              <td>GET</td>
              <td>/products</td>
              <td>
                <Button onClick={() => handler(fetchProducts)}>Send request</Button>
              </td>
            </tr>
            <tr>
              <td>GET</td>
              <td>/customers</td>
              <td>
                <Button onClick={() => handler(fetchCustomers)}>Send request</Button>
              </td>
            </tr>
            <tr>
              <td>GET</td>
              <td>/connections/access_token</td>
              <td>
                <Button onClick={() => handler(fetchConnection)}>Send request</Button>
              </td>
            </tr>
          </tbody>
        </Table>
        ...
        {errorMessage != '' && (
          <div style={{ marginTop: 4 }}>
            <Form.Control
              ...
              rows={40}
              cols={80}
              value={JSON.stringify(errorMessage.response.data.error, undefined, 4)}
            />
          </div>
        )}
      </div>
